...
import com.anf.domain.social.FriendsRequestService;
import com.anf.domain.social.MessagesService;
...
public class CommunicationController {
  ...
  /**
   * Sends a message. Receives two Strings (receiver login and Message itself), takes sender object
   * from SecurityContext
   */
  @PostMapping("/profile/messages")
  public ResponseEntity<String> sendMessage(
      @RequestParam("message") String message, @RequestParam("receiver") String receiver) {
    try {
      User recvr = userService.getUser(receiver);
      if (recvr == null)
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body("{\"User " + receiver + " wasn't found.\"}");
      User sender =
      ...
      String wsmessage =
          SecurityContextHolder.getContext().getAuthentication().getName() + ":" + message;
  ...
  public ResponseEntity<?> getMessagesFromDialog(@RequestParam String secondName) {
    try {
      ...
      if (rec == null)
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body("{\"User with name " + secondName + " wasn't found.\"}");
  ...
  public ResponseEntity<String> deleteMessage(@PathVariable int id) {
    try {
      ...
      if (msg == null)
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body("{\"Message with id = " + id + " wasn't found.\"}");
      if (msg.getSender().equals(applier)) {
        messageService.removeMessage(id);
      } else {
        return ResponseEntity.status(HttpStatus.FORBIDDEN)
            .body("{\"User can only remove his own messages.\"}");
      }
    ...
    try {
      ...
      if (message.getReceiver().equals(recvr)) {
        ...
        return ResponseEntity.status(HttpStatus.OK)
            .body("{\"Message 'is read' state is confirmed.\"}");
      }
      return ResponseEntity.status(HttpStatus.FORBIDDEN)
          .body("{\"User can only set read status on messages (s)he received.\"}");
    } catch (Throwable error) {
  ...
  /**
   * Takes a user from SecurityContext and a User from username and removes either request from
   * *username* to current User or from current User to *username*. See @Query in
  ...
  public ResponseEntity<String> deleteRequest(
    ...
    try {
      ...
      if (recvr == null)
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body("{\"User with name " + username + " wasn't found.\"}");
  ...
  public ResponseEntity<String> sendRequest(@RequestParam String username) {
    try {
      String wsMessage =
          "request:+" + SecurityContextHolder.getContext().getAuthentication().getName();
  ...
  public ResponseEntity<String> addFriend(@RequestParam String login) {
    try {
      String wsMessage =
          "friend:+" + SecurityContextHolder.getContext().getAuthentication().getName();
      User acceptor =
          userService.getUser(SecurityContextHolder.getContext().getAuthentication().getName());
      ...
      if (reqId != -1) userService.addFriend(acceptor, sender);
      else
        return ResponseEntity.status(HttpStatus.NOT_FOUND)
            .body("{\"No friend request was found.\"}");
      ...
      return ResponseEntity.status(HttpStatus.CREATED)
          .body("{\"Friends relationship is created.\"}");
    } catch (Throwable error) {
      return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(error.getMessage());
    }
  }
  ...
  //    public ResponseEntity<String> inviteToFight(@RequestParam String type, @RequestParam String
  // username) {
